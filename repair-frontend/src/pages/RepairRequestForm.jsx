      ...
      {!user ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full p-2 border border-body rounded-md" required />
          </div>
          <div>
            <label className="block text-heading font-medium mb-1">Last Name</label>
            <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full p-2 border border-body rounded-md" required />
          </div>
          <div>
            <label className="block text-heading font-medium mb-1">Contact Number</label>
            <input type="text" value={contactNumber} onChange={(e) => setContactNumber(e.target.value)} className="w-full p-2 border border-body rounded-md" required />
          </div>
          <div>
            <label className="block text-heading font-medium mb-1">Address</label>
            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} className="w-full p-2 border border-body rounded-md" required />
          </div>
          <div>
            <label className="block text-heading font-medium mb-1">Damage Type</label>
            <input type="text" value={damageType} onChange={(e) => setDamageType(e.target.value)} className="w-full p-2 border border-body rounded-md" placeholder="Enter damage type" required />
          </div>
          <button type="submit" disabled={loading} className="w-full bg-secondary text-white py-2 rounded-xl font-medium hover:bg-primaryHover transition">
